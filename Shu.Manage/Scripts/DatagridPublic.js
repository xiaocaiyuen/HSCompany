...
/*Id:         主键Id值
/*setkey:     其他自定义的相关的字段值
/*buttonJson: 操作按钮相关的值（json 格式）
--------------------------------------------------*/
function OperatingButton(url, id, setkey, buttonJson) {
    buttonJson = buttonJson.replace(/\|/g, '"');
    var Buttonjson = JSON.parse(buttonJson);//json序列化
    if (!jQuery.isEmptyObject(Buttonjson)) {
        var parm = 'active=' + Buttonjson.Marker + '&id=' + id + setkey;//链接参数
        var EventType = Buttonjson.EventType;//按钮事件类型
        var BId = Buttonjson.Marker;//按钮标识
        var BUrl = Buttonjson.Url;//按钮跳转的URL
        var strUrl = url.substring(url.indexOf("?") + 1, url.length);
        if (BUrl != "" && BUrl != null) {
            if (BUrl.indexOf('?') > 0) {
                BUrl = BUrl + "&" + parm + "&" + strUrl;
            } else {
                BUrl = BUrl + "?" + parm + "&" + strUrl;
            }
        }
        if (EventType == 1) {//链接
            location.href = BUrl;
        } else if (EventType == 4) {//新页面
            window.parent.addTab('详细-' + Buttonjson.Name, BUrl, '');
        } else if (EventType == 2) {//事件
            showConfirmMsg("确定要" + Buttonjson.Name + "吗？", function (r) {
                if (r) {
                    getAjax(url, parm, function (rs) {
                        if (rs == "0") {
                            showAlertMsg(Buttonjson.Name + "失败！");
                        }
                        else if (rs == "1") {
                            showAlertMsgUrl(Buttonjson.Name + "成功！", url);
                        }
                        else if (rs == 2) {
                            showAlertMsg("该数据被关联,无法" + Buttonjson.Name + "！");
                        }
                        else if (rs == 3) {
                            showAlertMsg("此数据不可以被" + Buttonjson.Name + "！");
                        }
                        else {
                            showAlertMsg(rs);
                        }
            ...
            $('#ui_' + BId).css('display', '');
            $('#ui_' + BId).dialog('open');
            $('#view_' + BId).attr('src', BUrl);
        }
    }
    else {
        showAlertMsg('方法有误，请联系管理员！');
    }
}
