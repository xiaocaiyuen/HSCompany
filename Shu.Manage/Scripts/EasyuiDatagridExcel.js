        ...
        for (var i = 0, it = rows, l = it.length; i < l; i++) {
            ...
            for (var j = 0; j < colCount; j++) {
                ...
                if (cfs[j] != '') {
                    ...
                    if (cellType[k] !== "None") {
                        if (param.id != j) {
                            t += '<ss:Cell ss:StyleID="' + cellClass + cellTypeClass[k] + '"><ss:Data xmlns="http://www.w3.org/TR/REC-html40" ss:Type="' + cellType[k] + '">';
            ...
            t += '</ss:Row>';
            if (param.id != null) {
                var v = r[cfs[param.id]];
                if (v != null && v != '' && v != undefined)
                if (v.indexOf('<br/>') >= 0) {
                    $.each(v.split('<br/>'), function (index, value) {
                        t += children(value, colCount, cfs, cellType, cellTypeClass, param);
                    });
                }
            }
        }
...
function children(value, colCount, cfs, cellType, cellTypeClass, param) {
    var t = '';
    if (value != undefined) {
        t += '<ss:Row>';
        //var cellClass = (i & 1) ? 'odd' : 'even';
            ...
            if (cfs[j] != '') {
                if (cellType[k] !== "None") {
                    ...
                    if (param.id == j) {
                        ...
                        t += '<ss:Cell ss:StyleID="'  + cellTypeClass[k] + '"><ss:Data ss:Type="' + cellType[k] + '">';
                        if (cellType[k] == 'DateTime') {
                            t += value.format('Y-m-d');
                        } else {
                            t += value;
                        }
                        t += '</ss:Data></ss:Cell>';
